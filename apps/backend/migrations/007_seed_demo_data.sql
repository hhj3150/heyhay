...
INSERT INTO customers (name, phone, email, channel, segment, address_zip, address_main, address_detail, marketing_sms, total_orders, total_spent, ltv, first_order_at, last_order_at) VALUES
  ...
  ('조예린', '010-2345-1013', 'cho.yr@email.com', 'SMARTSTORE', 'PAUSED', '35204', '대전 서구 둔산로 100', '801호', true, 4, 64000, 70000, '2026-01-10', '2026-02-28'),
...
-- 알림 데모 (P1~P3)
-- ============================================================
INSERT INTO alerts (module, priority, title, message, target_roles) VALUES
  ('farm', 'P1', '착유량 급감 경고', '별이(002-1001) 착유량이 어제 대비 -32% 감소했습니다. 건강 확인이 필요합니다.', ARRAY['ADMIN','FARM']),
  ('factory', 'P1', 'CCP1 온도 이탈', '배치 20260322-A2-001: 살균 온도 70.5°C → 기준 72°C 미달. 즉시 확인 필요.', ARRAY['ADMIN','FACTORY']),
  ('market', 'P2', '재고 안전선 도달', 'A2 저지우유 750ml 재고 12개 → 안전선(20개) 이하. 생산 계획 확인.', ARRAY['ADMIN','FACTORY']),
  ('market', 'P2', '결제 실패 2건', '김서연, 이준호 고객 정기구독 결제 실패. 24시간 내 재시도 예정.', ARRAY['ADMIN']),
  ('farm', 'P3', '분만 예정 D-5', '겨울이(002-1008) 분만 예정일 2026-03-27. 분만실 준비 필요.', ARRAY['ADMIN','FARM']),
  ('factory', 'P3', '경기도청 인증 갱신 D-25', 'HACCP 인증 만료 2026-04-16. 갱신 서류 준비 필요.', ARRAY['ADMIN','FACTORY'])
ON CONFLICT DO NOTHING;
